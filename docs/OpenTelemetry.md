...
`--aot` and `--otel` are independent template choices. Native AOT replaces the Lambda hosting/serialization boundary with `LambdaBootstrapBuilder` and source-generated JSON metadata; OpenTelemetry continues to wrap the inherited `FunctionHandlerAsync` with `AWSLambdaWrapper.TraceAsync`.
